...
include_once 'dbconnect.php';
	...
	if(!isset($_SESSION['user'])){	
		...
		$res=mysql_query("SELECT * FROM user,registered_user 
			...
			if($userRow['userID'] == $_SESSION['user'] ){
				...
				?>
				<script>alert('only normal users can see this page!! ');</script>
				<?php
				header("Location: index.php ");
			}
	}		
	...
	$userIDToFindFollowed = $_SESSION['userIDForFollowedAuthors'];
	$authorList = mysql_query("SELECT user.username, user.userID
								FROM user,author,follow
								WHERE author.userID = user.userID AND
									  follow.authorID = user.userID AND
									  follow.userID = '$userIDToFindFollowed'
								LIMIT 10
							");
	var_dump($authorList);
